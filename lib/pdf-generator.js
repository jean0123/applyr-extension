...
(function () {
  ...
  class ContentStream {
    ...
    drawBulletDot(x, y, radius = 1.5) {
      // Approximate circle with 4 Bezier curves
      const k = 0.5523 * radius; // magic number for circle approximation
      this._op(`${x} ${y + radius} m`);
      this._op(`${x + k} ${y + radius} ${x + radius} ${y + k} ${x + radius} ${y} c`);
      this._op(`${x + radius} ${y - k} ${x + k} ${y - radius} ${x} ${y - radius} c`);
      this._op(`${x - k} ${y - radius} ${x - radius} ${y - k} ${x - radius} ${y} c`);
      this._op(`${x - radius} ${y + k} ${x - k} ${y + radius} ${x} ${y + radius} c`);
      this._op('f'); // fill
    }
  ...
  function renderResume(data) {
      ...
      for (const job of data.experience) {
        ...
        if (job.bullets && job.bullets.length > 0) {
          for (const bullet of job.bullets) {
            cs.ensureSpace(24);
            const lines = wrapText(bullet, 10, CONTENT_W - 20);
            for (let li = 0; li < lines.length; li++) {
              cs.setColor(COLORS.text);
              if (li === 0) {
                // Draw bullet dot on first line only
                cs.drawBulletDot(MARGIN_L + 10, y() + 3, 1.5);
              }
              cs.text(lines[li], MARGIN_L + 18, y(), 10, 'F1');
              addY(-13);
            }
          }
    ...
    if (data.skills && data.skills.length > 0) {
      ...
      if (Array.isArray(data.skills)) {
        // Check if skills use category format ("Category: skill1, skill2")
        const hasCategories = data.skills.some(s => typeof s === 'string' && s.includes(':'));
        if (hasCategories) {
          for (const skillLine of data.skills) {
            cs.ensureSpace(14);
            const colonIdx = skillLine.indexOf(':');
            if (colonIdx > -1) {
              const category = skillLine.substring(0, colonIdx + 1);
              const items = skillLine.substring(colonIdx + 1).trim();
              cs.setColor(COLORS.text);
              cs.drawBulletDot(MARGIN_L + 10, y() + 3, 1.5);
              // Bold category label, then regular text
              cs.text(category, MARGIN_L + 18, y(), 10, 'F2');
              const catWidth = estimateTextWidth(category + ' ', 10);
              const remainingWidth = CONTENT_W - 18 - catWidth;
              if (remainingWidth > 50) {
                const itemLines = wrapText(items, 10, CONTENT_W - 18);
                for (let li = 0; li < itemLines.length; li++) {
                  if (li === 0) {
                    cs.text(itemLines[li], MARGIN_L + 18 + catWidth, y(), 10, 'F1');
                  } else {
                    addY(-13);
                    cs.ensureSpace(14);
                    cs.text(itemLines[li], MARGIN_L + 18, y(), 10, 'F1');
                  }
                }
              }
              addY(-13);
            } else {
              const lines = wrapText(skillLine, 10, CONTENT_W - 18);
              cs.drawBulletDot(MARGIN_L + 10, y() + 3, 1.5);
              for (const line of lines) {
                cs.setColor(COLORS.text);
                cs.text(line, MARGIN_L + 18, y(), 10, 'F1');
                addY(-13);
              }
            }
          }
        ...
        const skillLines = wrapText(data.skills, 10, CONTENT_W);
        for (const line of skillLines) {
          cs.ensureSpace(14);
          cs.setColor(COLORS.text);
          cs.text(line, MARGIN_L, y(), 10, 'F1');
    ...
    if (data.achievements && data.achievements.length > 0) {
      renderSection(cs, 'KEY ACHIEVEMENTS');
      for (const achievement of data.achievements) {
        cs.ensureSpace(24);
        const lines = wrapText(achievement, 10, CONTENT_W - 20);
        for (let li = 0; li < lines.length; li++) {
          cs.setColor(COLORS.text);
          if (li === 0) {
            cs.drawBulletDot(MARGIN_L + 10, y() + 3, 1.5);
    ...
    if (data.certifications && data.certifications.length > 0) {
      ...
      for (const cert of data.certifications) {
        ...
        cs.setColor(COLORS.text);
        cs.drawBulletDot(MARGIN_L + 10, y() + 3, 1.5);
        cs.text(certLine, MARGIN_L + 18, y(), 10, 'F1');
        addY(-13);
      }
      addY(-4);
    }
